...
def train(config: DictConfig) -> Optional[float]:
    ...
    if config.get("evaluate_untrained_model"):
        log.info("Evaluating untrained model")
    ...
    if config.get("test"):
        ...
        if not config.get("train") or config.trainer.get("fast_dev_run"):
            ckpt_path = None
        # We test five times to report mean and standard deviations using our backgound augmentations
        for i in range(5):
            log.info(f"Starting testing ({i+1}/5)!")
